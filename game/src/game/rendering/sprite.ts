...
export class Sprite {
	...
	constructor(image: Image, viewport: Viewport | undefined, isFlipped: boolean = false) {
		...
		this.originX = viewport?.originX ?? (viewport !== undefined ? viewport.width / 2 : image.getWidth() / 2);
		this.originY = viewport?.originY ?? (viewport !== undefined ? viewport.height / 2 : image.getHeight() / 2);
	}
